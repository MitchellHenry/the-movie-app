...
export async function getMovies(searchType, pageStart, pages, type, genreID)
{
    ...
    for (let currentPage = pageStart; currentPage < (pageStart + pages); currentPage++) {
        ...
        if(currentPage + 1 > parseInt(movieData.total_pages))
        {
            break;  //Incase the number of pages requested does not meet the number the API would provide.
                                    //20 Movies per page.
        }
    }
    return movies
...
//Get details on specific Movie details
